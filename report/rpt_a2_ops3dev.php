...
//    $x = $this->GetX();
//    $y = $this->GetY();
//      $serverName = "192.168.118.104";
//      $connectionInfo = array( "Database"=>"RMD3SCALE1", "UID"=>"sa", "PWD"=>"sipco77" , 'ReturnDatesAsStrings'=>true ,"CharacterSet" => "UTF-8");
 ...
 function fetch_data($rmd_date)
  {
     ...
     $serverName = "192.168.118.104";
      ...
      if( $conc === false ) {die( print_r( sqlsrv_errors(), true));}
            $sql4 = "SELECT convert(nvarchar(10), rmd_date, 23) as date,cast(rmd_length as decimal(18,2))/1000 as length,*
            FROM dbo.vw_A2_rpt where  rmd_date = '$rmd_date' order by rmd_date ";
     ...
     $output .='
         ...
         <td align="left"   width ="18%">&nbsp;'.$row4['rmd_size'].'&nbsp;</td>
         <td align="Right"  width ="13%">'.number_format($row4['length'],1).'&nbsp;</td>
         <td align="Center"   width ="8%">'.$row4['rmd_qa_grade'].'</td>
         <td align="Right"  width ="10%">'.number_format($row4['rmd_qty3']).'&nbsp;</td>
         <td align="right" width ="13%" >'.number_format($row4['rmd_weight']).'&nbsp;</td>
         <td align="left"   width ="20%">&nbsp;'.$row4['rmd_remark'].'</td>
         </tr>';
  ...
  function fetch_line()
  {
    ...
    $serverName = "192.168.118.104";
     ...
     $conc = sqlsrv_connect( $serverName, $connectionInfo  );
     if( $conc === false ) {die( print_r( sqlsrv_errors(), true));}
           $sql4 = "SELECT convert(nvarchar(10), rmd_date, 23) as rmdate
           FROM dbo.vw_A2_rpt where  LEFT(CONVERT(varchar, [rmd_date],23),7) = '$ref2' group by rmd_date ";
       $result4 = sqlsrv_query($conc, $sql4);
     //  $row4 = sqlsrv_fetch_array($result4, SQLSRV_FETCH_ASSOC);
    ...
    $refs=$ref;
     $id = 0;
     while ($row4 = sqlsrv_fetch_array($result4, SQLSRV_FETCH_ASSOC)) {
     $rmd_date = $row4['rmdate'];    
     $id ++;
    $output .='
        <tr bgcolor="#AAAAAA">
        <th align="Center" width ="100%" rowspan><b>วันที่ผลิต '.$row4['rmdate'].'</b></th>
        </tr>
        <tr bgcolor="#AAAAAA">
        ...
        <th align="Center" width ="13%" rowspan><b>ความยาว<br>(เมตร)</b></th>
        <th align="Center" width ="8%" rowspan ><b>เกรด</b></th>
        <th align="Center" width ="10%" rowspan ><b>จำนวน<br>(เส้น)</b></th>
        <th align="Center" width ="13%" rowspan ><b>น้ำหนัก(Kg.)</b></th>
        <th align="Center" width ="20%" rowspan ><b>หมายเหตุ</b></th>
        </tr>';
        

        $output.= fetch_data($row4['rmdate']);
     }
 ...
 function fetch_sum($ref)
   {
    ...
    if( $conn === false ) {die( print_r( sqlsrv_errors(), true));}
        $sql5 = "SELECT  sum(rmd_qty3) as sQty3,sum(rmd_weight) as sweight 
        FROM dbo.vw_A2_rpt where  LEFT(CONVERT(varchar, [rmd_date],23),7) = '$ref2'";
    ...
    $output='';
      ...
      <tr>
         <td align="right"   width ="57%" >รวมทั้งสิ้น&nbsp;</td>
         <td class = "x" align="Right"  width ="10%">&nbsp;'.number_format($rows5['sQty3']).'&nbsp;</td>
         <td align="Right"   width ="13%">'.number_format($rows5['sweight']).'&nbsp;</td>
 ...
 function fetch_overall($size,$ref2)
   {
    ...
    $conn = sqlsrv_connect( $serverName, $connectionInfo  );
    if( $conn === false ) {die( print_r( sqlsrv_errors(), true));}
        $sql5 = "SELECT  [rmd_size],[rmd_qa_grade],[rmd_remark],sum([rmd_qty3]) as sumqty3,sum([rmd_weight]) as sweight
          FROM [RMD3SCALE1].[dbo].[vw_A2_rpt]
      ...
      <tr>
         <td align="Left"   width ="22%" >&nbsp;'.$row5['rmd_size'].'</td>
